...
fileIn="2017-05-28-LKH-second-round.txt"
...
# data MEAN to plot
df_data_MEAN_sparse = df_data_MEAN.loc[df_data_MEAN['nPOIs'] == 40]
...
## data RAW to plot
df_data_RAW_sparse = df_data_RAW.loc[df_data_RAW['nPOIs'] == 40]
...
ax.legend(lines, labels, loc='best')
plt.title('tourSize/map - ESPARÇO')
...
ax.legend(lines, labels, loc='best')
plt.title('globalAvgDelay/map - ESPARÇO')
...
ax.set(xlabel="maps", ylabel="globalAvgDelay")
ax.legend(lines, labels, loc='best')
plt.title('globalAvgDelay/map - DENSO')
