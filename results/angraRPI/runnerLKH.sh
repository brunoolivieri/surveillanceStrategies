...
#./woofyMSG.sh "[$HOST][$TIME] Serao $TOTAL_DE_LOOPS das LKH)"

echo "[$HOST][$TIME] Serao $TOTAL_DE_LOOPS das LKH no RPI; começou " >> ./simulationResults/acompanhamento.txt

for ((i=ULTIMOMAPA; i<=MAPS; i++)); 
do
	for N_POI in 700 140 70
	do
		for SNAME in ZigZagOverLKHMobility  
		do
			TIME=$(date)
	...
	#./woofyMSG.sh "[$HOST][$TIME] Fim das LKH-crua x 100 com  $N_POI CHs no RPI, indo para o próximo "

done

./woofyMSG.sh "[$HOST][$TIME] Fim das LKH-crua x 100 no RPI  ... "
echo "[$HOST];[$TIME]; terminou as LKH-crua x 100 no RPI" >> ./simulationResults/acompanhamento.txt
...
do
	for N_POI in 700 140 70
	do
		for SNAME in ZigZagOverLKHCuttedMobility  
		do
			TIME=$(date)
			#echo "$TIME  -->  nPOI = "$N_POI"  strat = "$SNAME"  map = "$i   >> ./simulationResults/acompanhamento.txt
	...
	#./woofyMSG.sh "[$HOST][$TIME] Fim das LKH-CUT x 100 com  $N_POI CHs no RPI, indo para o próximo "

done

TIME=$(date)
./woofyMSG.sh "[$HOST][$TIME] Fim das LKH-cut x 100 no RPI  ... "
echo "[$HOST];[$TIME]; terminou as LKH-cut x 100 no RPI" >> ./simulationResults/acompanhamento.txt
