...
echo "[$HOST][$TIME] Serao $TOTAL_DE_LOOPS das LKH; começou " >> ./simulationResults/acompanhamento.txt

for N_POI in 70 140 700
do
	for ((i=ULTIMOMAPA; i<=MAPS; i++)); 
	do
		for SNAME in ZigZagOverLKHMobility ZigZagOverLKHCuttedMobility  
		do
			TIME=$(date)
	...
	./woofyMSG.sh "[$HOST][$TIME] Fim das LKH x 200 com  $N_POI CHs, indo para o próximo "
...
TIME=$(date)
./woofyMSG.sh "[$HOST][$TIME] Fim das LKH x 200  ... "
echo "[$HOST];[$TIME]; terminou as LKH x 200" >> ./simulationResults/acompanhamento.txt
